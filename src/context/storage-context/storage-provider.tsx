...
import { coalescedSave } from '@/lib/coalesced-save';
...
import { mergeBranchDiagram } from '@/lib/local-branch-merge';
import {
...
export const StorageProvider: React.FC<React.PropsWithChildren> = ({
    ...
    const db = useMemo(() => {
        const dexieDB = new Dexie('ChartDB') as Dexie & {
            repository_layouts: EntityTable<RepositoryLayout, 'id'>;
            saveRepositoryLayout: (diagramId: string) => Promise<void>;
        ...
        type LocalDB = typeof dexieDB;
        dexieDB.version(14).stores({ repository_layouts: 'id' });
        const repositoryIds = new Map<string, string>();
        const readLayoutDiagram = async (
            diagramId: string,
            sourceDB: LocalDB = dexieDB
        ): Promise<Diagram | undefined> => {
            const diagram = await sourceDB.diagrams.get(diagramId);
            if (!diagram) return undefined;
            return {
                ...diagram,
                tables: await sourceDB.db_tables
                    .where('diagramId')
                    .equals(diagramId)
                    .toArray(),
                areas: await sourceDB.areas
                    .where('diagramId')
                    .equals(diagramId)
                    .toArray(),
                notes: await sourceDB.notes
                    .where('diagramId')
                    .equals(diagramId)
                    .toArray(),
            };
        };
        dexieDB.saveRepositoryLayout = coalescedSave(async (diagramId) => {
            const repositoryId = repositoryIds.get(diagramId);
            if (!repositoryId) return;
            await dexieDB.transaction(
                'rw',
                [
                    dexieDB.repository_layouts,
                    dexieDB.diagrams,
                    dexieDB.db_tables,
                    dexieDB.areas,
                    dexieDB.notes,
                ...
                async () => {
                    const diagram = await readLayoutDiagram(diagramId);
                    if (diagram)
                        await dexieDB.repository_layouts.put(
                            captureRepositoryLayout(
                                repositoryId,
                                diagram,
                                await dexieDB.repository_layouts.get(
                                    repositoryId
                                )
                            )
                        );
        ...
        dexieDB.on('ready', async (readyDB) => {
            const dexieDB = readyDB as LocalDB;
            if (import.meta.env.VITE_LOCAL_ERD === 'true') {
                const response = await fetch('/data/snapshots.json', {
                    cache: 'no-store',
                });
                if (!response.ok)
                    throw new Error('Local ERD snapshots could not be loaded');
                ...
                for (const snapshot of payload.snapshots) {
                    if (snapshot.repositoryUrl)
                        repositoryIds.set(
                            snapshot.diagram.id,
                            repositoryKey(snapshot.repositoryUrl)
                        );
                }
                // Seed from existing browser work, before importing server defaults.
                const existing = await dexieDB.diagrams.toArray();
                existing.sort(
                    (a, b) =>
                ...
                const openingId = decodeURIComponent(
                    window.location.pathname.replace(/^\/diagrams\//, '')
                );
                existing.sort(
                    (a, b) =>
                        Number(a.id === openingId) - Number(b.id === openingId)
                );
                const seeded = new Map<string, RepositoryLayout>();
                for (const saved of existing) {
                    const repo = repositoryIds.get(saved.id);
                    if (!repo) continue;
                    const stored = await dexieDB.repository_layouts.get(repo);
                    if (stored?.version === 2) continue;
                    let diagram = await readLayoutDiagram(saved.id, dexieDB);
                    // Recover a coherent table/Area snapshot, never union independent layouts.
                    const backup = localStorage.getItem(
                        `debut-repo-layout-migration:${saved.id}`
                    );
                    if (stored && backup) {
                        try {
                            const original = JSON.parse(backup) as Diagram;
                            if (
                                original.id === saved.id &&
                                Array.isArray(original.tables)
                            )
                                diagram = original;
                        } catch {
                            // Keep current work when an old backup cannot be read.
                        }
                    } else if (stored) {
                        // Do not use an already merged diagram to repair another backup.
                        continue;
                    }
                    if (diagram) {
                        if (!backup)
                        ...
                        seeded.set(
                            repo,
                            captureRepositoryLayout(repo, diagram)
                        );
                    }
                }
                await dexieDB.repository_layouts.bulkPut([...seeded.values()]);
                for (const snapshot of payload.snapshots) {
                    const incoming = snapshot.diagram;
                    const importRevision = `v2:${snapshot.revision}`;
                    const revisionKey = `debut-erd-revision:${incoming.id}`;
                    if (
                        localStorage.getItem(revisionKey) === importRevision &&
                        (await dexieDB.diagrams.get(incoming.id))
                    )
                        continue;
                    incoming.createdAt = new Date(incoming.createdAt);
                    incoming.updatedAt = new Date(incoming.updatedAt);
                    const stored = await dexieDB.diagrams.get(incoming.id);
                    const previous = stored
                        ? {
                              ...stored,
                              tables: await dexieDB.db_tables
                                  .where('diagramId')
                                  .equals(incoming.id)
                                  .toArray(),
                              relationships: await dexieDB.db_relationships
                                  .where('diagramId')
                                  .equals(incoming.id)
                                  .toArray(),
                              areas: await dexieDB.areas
                                  .where('diagramId')
                                  .equals(incoming.id)
                                  .toArray(),
                              notes: await dexieDB.notes
                                  .where('diagramId')
                                  .equals(incoming.id)
                                  .toArray(),
                          }
                    ...
                    if (previous)
                        localStorage.setItem(
                            `debut-erd-backup:${incoming.id}`,
                            JSON.stringify(previous)
                        );
                    const merged = mergeBranchDiagram(incoming, previous);
                    await dexieDB.transaction(
                        'rw',
                        [
                            dexieDB.diagrams,
                            dexieDB.db_tables,
                            dexieDB.db_relationships,
                            dexieDB.areas,
                        ],
                        async () => {
                            await dexieDB.db_tables
                                .where('diagramId')
                                .equals(incoming.id)
                                .delete();
                            await dexieDB.db_relationships
                                .where('diagramId')
                                .equals(incoming.id)
                                .delete();
                            await dexieDB.diagrams.put({
                                id: merged.id,
                                name: merged.name,
                                databaseType: merged.databaseType,
                                createdAt: merged.createdAt,
                                updatedAt: merged.updatedAt,
                            });
                            await dexieDB.db_tables.bulkPut(
                                (merged.tables ?? []).map((t) => ({
                                    ...t,
                                    diagramId: incoming.id,
                                }))
                            );
                            await dexieDB.db_relationships.bulkPut(
                    ...
                    localStorage.setItem(revisionKey, importRevision);
                }
                const activeId = decodeURIComponent(
                    window.location.pathname.replace(/^\/diagrams\//, '')
                );
                const activeRepo = repositoryIds.get(activeId);
                if (activeRepo) {
                    const diagram = await readLayoutDiagram(activeId, dexieDB);
                    const layout =
                        await dexieDB.repository_layouts.get(activeRepo);
                    if (diagram && layout) {
                        localStorage.setItem(
                            `debut-repo-layout-backup:${activeId}`,
                            JSON.stringify(diagram)
                        );
                        const shared = applyRepositoryLayout(diagram, layout);
                        await dexieDB.transaction(
                            'rw',
                            [dexieDB.db_tables, dexieDB.areas, dexieDB.notes],
                            async () => {
                                await dexieDB.db_tables.bulkPut(
                                    (shared.tables ?? []).map((t) => ({
                                        ...t,
                    ...
                    const current = await readLayoutDiagram(activeId, dexieDB);
                    if (current)
                        await dexieDB.repository_layouts.put(
                            captureRepositoryLayout(activeRepo, current, layout)
                        );
                }
            }
    ...
    const addTable: StorageContext['addTable'] = useCallback(
        async ({ diagramId, table }) => {
            ...
            await db.saveRepositoryLayout(diagramId);
        },
        [db]
    );
    ...
    const updateTable: StorageContext['updateTable'] = useCallback(
        async ({ id, attributes }) => {
            ...
            const saved = await db.db_tables.get(id);
            if (saved) await db.saveRepositoryLayout(saved.diagramId);
        },
        [db]
    );
    ...
    const updateArea: StorageContext['updateArea'] = useCallback(
        async ({ id, attributes }) => {
            ...
            const saved = await db.areas.get(id);
            if (saved) await db.saveRepositoryLayout(saved.diagramId);
        },
        [db]
    );
