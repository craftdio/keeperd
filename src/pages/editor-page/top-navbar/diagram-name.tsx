...
export const DiagramName: React.FC<DiagramNameProps> = () => {
    ...
    const openRepositorySwitcher = useCallback(() => {
        ...
        <div className="group">
            ...
            >
                <DiagramIcon
                    ...
                    onClick={(e) => {
                        e.stopPropagation();
                        if (localERD) openRepositorySwitcher();
                        else openOpenDiagramDialog({ canClose: true });
                    }}
                />
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    {localERD ? (
                                        <button
                                            type="button"
                                            className="flex max-w-[340px] items-center gap-1 rounded px-1 py-0.5 hover:bg-background/50"
                                            aria-label={`${diagramName} 저장소 또는 브랜치 변경`}
                                            onClick={openRepositorySwitcher}
                                        >
                                            <h1
                                                className={cn(
                                                    labelVariants(),
                                                    'max-w-[300px] truncate group-hover:underline'
                                                )}
                                            >
                                                {diagramName}
                                            </h1>
                                            <ChevronDown className="size-3.5 flex-none text-slate-500" />
                                        </button>
                                    ) : (
                                        <h1
                                            className={cn(
